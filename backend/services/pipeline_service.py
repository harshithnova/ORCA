...
def parse_query_mvp(
    ...
    Extracts location and time_window from known MVP query patterns.
    ...
    if ref_time.tzinfo is None:
        ref_time = ref_time.replace(tzinfo=timezone.utc).astimezone(IST)
    else:
    ...
    start_dt = datetime(target_date.year, target_date.month, target_date.day, start_hour, 0, 0, tzinfo=IST)
    end_dt = datetime(target_date.year, target_date.month, target_date.day, end_hour, 0, 0, tzinfo=IST)
...
def run_pipeline(
    ...
    Args:
        request: PipelineRequest with query, optional location, and time_window.
        marine_record: Optional marine data override (for testing/injection).
        weather_record: Optional weather data override (for testing/injection).
        restricted_zones: Optional restricted zones override (for testing/injection).
        candidates: Optional candidate zones override (for testing/injection).
        current_time: Optional reference time for relative date parsing and freshness.

    Returns:
        Populated ReasonResponse matching API_CONTRACT.md.
    """
    # 1. Parse or normalize location and time window relative to current_time
    parsed = parse_query_mvp(request.query, now=current_time)

    req_loc = request.location or {}
    region = req_loc.get("region") or req_loc.get("name") or parsed["region"]
    lat = req_loc.get("latitude") if req_loc.get("latitude") is not None else parsed["latitude"]
    lon = req_loc.get("longitude") if req_loc.get("longitude") is not None else parsed["longitude"]

    time_window = request.requested_time or parsed["time_window"]

    # 2. Safety guard: Unsupported region check
    # Do NOT silently fall back to Kochi if an unsupported region was requested
    if region.lower() != "kochi":
        return ReasonResponse(
            status="NO_SAFE_RECOMMENDATION",
            query=request.query,
            location=LocationModel(name=region, latitude=lat, longitude=lon),
            ...
            evidence=[{
                ...
                "region": region,
                "reason": (
                    f"No normalized marine/weather data or candidate fixtures available for region '{region}'. "
    ...
    loc_dict = {"name": region, "region": region, "latitude": lat, "longitude": lon}
    ...
    if not raw_candidates:
        return ReasonResponse(
            ...
            evidence=[{
                ...
                "reason": "No active candidate zones found within the specified search radius.",
            }],
            map=None,
        )

    # 5. Apply spatial filtering
    passed, blocked = apply_spatial_filter(raw_candidates, restricted_zones)
    blocked_map = {c["id"]: c for c in blocked if "id" in c}

    annotated_candidates = []
    for c in raw_candidates:
        cid = c.get("id")
        if cid in blocked_map:
            annotated_candidates.append(blocked_map[cid])
        else:
            c_annotated = dict(c)
            c_annotated["spatial_blocked"] = False
            annotated_candidates.append(c_annotated)

    # 6. Replan Manager initialization (ranked order preserved)
    replan = ReplanManager(annotated_candidates)

    # 7. Evaluate candidates in ranking order
    target_dt = parse_iso_datetime(time_window.get("valid_from")) or datetime.now(timezone.utc)
    ref_now = current_time or datetime.now(timezone.utc)

    # Check forecast validity & retrieval freshness
    fresh_ok, fresh_diag = check_marine_and_weather_freshness(
        marine_record, weather_record, target_time=target_dt, current_time=ref_now
    )
    ...
    while not replan.is_exhausted():
        ...
        suit_res = calculate_suitability(marine_record, weather_record)
        risk_res = calculate_risk(marine_record, weather_record)
        conf_res = calculate_confidence(marine_record, weather_record, query_time=target_dt)
        ...
        status = safety_res.get("safety_status", "BLOCK")

        if status == "BLOCK":
            # Record block in ReplanManager and loop to next candidate
            replan.record_evaluation(
                candidate_id=cid,
                safety_status="BLOCK",
                blocking_reason=safety_res.get("blocking_reason"),
                blocking_evidence=safety_res.get("blocking_evidence", []),
            )
            continue

        # Candidate is SAFE or CAUTION -> Valid recommendation found!
        # Rule: CAUTION is a valid final status and must NOT trigger replanning
        replan.record_evaluation(
            candidate_id=cid,
        ...
        for ev in safety_res.get("blocking_evidence", []):
            evidence_list.append(ev)

        if marine_record:
            evidence_list.append({
                "source": marine_record.get("source", "INCOIS"),
                "data_mode": marine_record.get("data_mode", "CACHED_OFFICIAL"),
                "data_type": marine_record.get("data_type", "FORECAST"),
                "valid_from": marine_record.get("valid_from"),
                "wave_height_m": marine_record.get("wave_height_m"),
            })
        ...
        if weather_record:
            evidence_list.append({
                ...
                "data_type": weather_record.get("data_type", "FORECAST"),
                "warning_level": weather_record.get("warning_level"),
            })
        ...
        # Reason text
        if status == "SAFE":
            reason_text = "Candidate passes all spatial, weather, and safety constraint checks."
        else:
            caution_reasons = [
                ev.get("reason")
                for ev in safety_res.get("blocking_evidence", [])
            ...
            if caution_reasons:
                reason_text = f"Candidate recommended with CAUTION: {'; '.join(caution_reasons)}"
            else:
                reason_text = "Candidate recommended with CAUTION based on current environmental advisories."

        # Map GeoJSON
        map_model = None
        geom = candidate.get("geometry")
        if geom:
            map_model = MapModel(geojson={
                "type": "Feature",
                "geometry": geom,
                "properties": {
                    "zone_id": cid,
    ...
    # 8. All candidates blocked / exhausted -> NO_SAFE_RECOMMENDATION
    ...
    for entry in replan.audit_log:
        audit_evidence.append({
            ...
            "evidence": entry["blocking_evidence"],
        })
